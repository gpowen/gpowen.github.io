...
# Load in CSV
tweets.raw <- read.csv("MeToo_tweets.csv", stringsAsFactors = FALSE)
View(tweets.raw)

# Clean up, filter
tweets.raw <- tweets.raw[, 1:5]
View(tweets.raw)

# Missing values?
length(which(!complete.cases(tweets.raw))) # Nope.

# Subsetting and merging
tweets.dropped <- subset (tweets.raw, Source=="Twitter for iPhone")
tweets.dropped2 <- subset (tweets.raw, Source=="Twitter for Android")
tweets.merged <- rbind(tweets.dropped, tweets.dropped2)

# Set as Factor
tweets.merged$Source <- as.factor(tweets.merged$Source)

# Explore the data; distribution
prop.table(table(tweets.merged$Source))

# Lengths of Tweets
summary(tweets.merged$Length)

# Tweet Lengths

library(ggplot2)

ggplot(tweets.merged, aes(x = Length, fill = Source)) +
  theme_bw() +
  geom_histogram(binwidth = 5) +
  labs(y = "Count", x = "Length of Tweets",
       title = "Distribution of Tweet Length by Source")
...
# seed for reproducibility. (Keep 70% to build the model, save 30% to test the model; 
set.seed(32984)
indexes <- createDataPartition(tweets.merged$Source, times = 1,
                               p = 0.7, list = FALSE)

train <- tweets.merged[indexes,]
test <- tweets.merged[-indexes,]


# Verify proportions.
prop.table(table(train$Source))
prop.table(table(test$Source))
...
dim(train.tokens.matrix) # Shows increased amount of columns
...
train.tokens.df <- cbind(Label = train$Source, data.frame(train.tokens.dfm))
...
set.seed(48743)
cv.folds <- createMultiFolds(train$Source, k = 10, times = 3)
...
# Total time of execution approx. 40 seconds.
...
# Make a clean data frame using the same process as before; proceed with N-grams?
train.tokens.tfidf.df <- cbind(Label = train$Tweet, data.frame(train.tokens.tfidf))
